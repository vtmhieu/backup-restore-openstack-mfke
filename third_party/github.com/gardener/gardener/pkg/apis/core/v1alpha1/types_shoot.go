...
type ShootSpec struct {
	...
	//LoadBalancerType is the type of load balancer to create kube-api server and vpn service
	LoadBalancerType string `json:"loadBalancerType,omitempty" protobuf:"bytes,21,opt,name=loadBalancerType"`
	// TransactionId is transcaction id of transaction in portal user
	TransactionId string `json:"transactionId,omitempty" protobuf:"bytes,22,opt,name=transactionId"`
	// UsePrivateWan checks if cluster use private wan for vpn shoot LB
	UsePrivateWan *string `json:"usePrivateWan,omitempty" protobuf:"bytes,23,opt,name=usePrivateWan"`
	// PublicKey is user's public key for access to shoot worker node
	PublicKey *string `json:"publicKey,omitempty" protobuf:"bytes,24,opt,name=publicKey"`
	// AutoUpgrade is config for upgrade time windown
	// +optional
	AutoUpgrade *AutoUpgrade `json:"autoUpgrade,omitempty" protobuf:"bytes,25,opt,name=autoUpgrade"`
}
...
type ShootStatus struct {
	...
	// GPUSharingConfig holds information about gpu sharing config of shoot cluster (single/mixed/none/MPS)
	// +optional
	GPUSharingConfig string `json:"gpuSharingConfig,omitempty" protobuf:"bytes,19,opt,name=gpuSharingConfig"`
	// TransactionId holds information about operation in Portal
	// +optional
	TransactionId *string `json:"transactionId,omitempty" protobuf:"bytes,20,opt,name=transactionId"`
	// TransactionStatus holds information about operation in Portal (Processing || Succeeded || Failed)
	// +optional
	TransactionStatus string `json:"transactionStatus,omitempty" protobuf:"bytes,21,opt,name=transactionStatus"`
}
...
type Addons struct {
	...
	// GpuOperator holds configuration settings for the gpu-operator addon.
	// +optional
	GpuOperator *GpuOperator `json:"gpuOperator,omitempty" protobuf:"bytes,3,opt,name=gpuOperator"`
}

type AutoUpgrade struct {
	// TimeUpgrade holds the specified time to allow auto upgrade cluster
	// +optional
	TimeUpgrade []string `json:"timeUpgrade,omitempty" protobuf:"bytes,1,opt,name=timeUpgrade"`
	// TimeZone holds the time zone
	// +optional
...
type GpuOperator struct {
	...
	MigConfig MigConfig `json:"migConfig,omitempty" protobuf:"bytes,2,opt,name=migConfig"`
	MPS       MpsConfig `json:"mpsConfig,omitempty" protobuf:"bytes,3,rep,name=mpsConfig"`
}

type MpsConfig struct {
	// Maxclient is a map of key/value pairs for maxclient of mps in each worker group
	// +optional
	Maxclient []string `json:"maxClient,omitempty" protobuf:"bytes,1,rep,name=maxClient"`
}

type MigConfig struct {
	Strategy string `json:"strategy,omitempty" protobuf:"bytes,1,opt,name=strategy"`
}
type PrometheusGPU struct {
	Addon `json:",inline" protobuf:"bytes,1,opt,name=addon"`
}
...
type KubeAPIServerConfig struct {
	...
	// SourceRanges contains array of source range ip that can connect to the api server
	// +optional
	SourceRanges []string `json:"sourceRanges,omitempty" protobuf:"bytes,14,opt,name=sourceRanges"`
}
