...
func ValidateShootSpec(meta metav1.ObjectMeta, spec *core.ShootSpec, fldPath *field.Path, inTemplate bool) field.ErrorList {
	...
	if len(spec.LoadBalancerType) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("loadBalancerType"), "must specify a type of load balancer"))
	}
...
func validateNetworking(networking core.Networking, fldPath *field.Path) field.ErrorList {
	...
	if networking.Nodes != nil {
		nodeNetworks := strings.Split(*networking.Nodes, ",")
		for _, nodeNetwork := range nodeNetworks {
			path := fldPath.Child("nodes")
			cidr := cidrvalidation.NewCIDR(nodeNetwork, path)
